...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const handleExit = useCallback(() => {
    if (confirm('Are you sure you want to close Music Manager?')) {
      window.close();
      alert('In a browser, window.close() may be restricted. Please close the tab.');
    }
  }, []);
  ...
  const Sidebar = (
    <aside className="w-64 border-r border-slate-800 bg-slate-900/50 flex flex-col z-50">
      ...
      <div className="p-4 border-t border-slate-800 space-y-2">
        ...
        <div className="px-4 py-2 flex items-center justify-between">
          ...
          <div className="relative">
            ...
            />
            {notificationCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-[rgb(var(--accent-rgb))] text-white text-[9px] font-bold flex items-center justify-center rounded-full border-2 border-slate-900 animate-pulse">
  ...
  return (
    <div className={`flex flex-col h-screen bg-[var(--bg-color)] text-slate-200 overflow-hidden ${sidebarPosition === 'right' ? 'flex-row-reverse' : ''}`}>
      {/* Top Menu Bar */}
      <div className="h-8 bg-slate-900 border-b border-slate-800 flex items-center px-4 gap-4 z-[60] fixed top-0 left-0 right-0">
      ...
      <div className="flex flex-1 overflow-hidden mt-8 relative">
        {Sidebar}
        ...
        <div className="flex-1 relative flex flex-col bg-[var(--bg-color)] overflow-hidden">
          ...
          {/* Status Bar */}
          <footer className="h-6 bg-slate-900 border-t border-slate-800 flex items-center px-4 justify-between z-[60]">
            ...
            <div className="flex items-center gap-4 text-[10px] text-slate-600 font-mono">
              ...
              <span className="flex items-center gap-1"><Monitor className="w-3 h-3" /> DPI_STABLE</span>
            </div>
          </footer>
        </div>
      </div>
